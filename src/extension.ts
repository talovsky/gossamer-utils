...
import { wrapSelectionInFragment, wrapSelectionInHook } from "./wrapSelection";
import {
...
const fragmentActionLanguages = new Set(["javascriptreact", "typescriptreact"]);
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const provider: vscode.RenameProvider = {
    prepareRename(document, position) {
      ...
      const activeRange =
        offset >= tuple.getterRange.start && offset <= tuple.getterRange.end
          ? tuple.getterRange
          : tuple.setterRange;
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      "gossamer-utils.jumpToMatchingBracketOrTag",
      jumpToMatchingBracketOrTag,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.jumpToMatchingTag",
      jumpToMatchingTag,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.removeTagPreserveChildren",
      removeTagPreserveChildren,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.balanceTagInward",
      balanceTagInward,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.balanceTagOutward",
      balanceTagOutward,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.renameSymbol",
      renameSymbol,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.goToDefinition",
      goToDefinition,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.convertFunctionToArrow",
      convertFunctionToArrow,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.convertArrowToFunction",
      convertArrowToFunction,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.wrapSelectionWithTag",
      wrapSelectionWithTag,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.wrapSelectionInUseMemo",
      () => wrapActiveSelectionInHook("useMemo"),
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.wrapSelectionInUseCallback",
      () => wrapActiveSelectionInHook("useCallback"),
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.wrapSelectionInFragment",
      wrapActiveSelectionInFragment,
    ),
    vscode.commands.registerCommand("gossamer-utils.tabout", tabOut),
    vscode.commands.registerCommand(
      "gossamer-utils.taboutReverse",
      tabOutReverse,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.toggleTabOut",
      toggleTabOut,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.hungryDelete",
      hungryDelete,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.showOpenedEditors",
      showOpenedEditors,
    ),
    vscode.commands.registerCommand(
      "gossamer-utils.showQuickOpenWithPath",
      showQuickOpenWithPath,
    ),
  );
}
...
async function wrapActiveSelectionInHook(wrapper: "useMemo" | "useCallback") {
  ...
  await editor.edit((builder) => {
    builder.replace(
      editor.selection,
      wrapSelectionInHook(selectedText, wrapper),
    );
  });
}
...
async function wrapActiveSelectionInFragment() {
  ...
  await editor.edit((builder) => {
    builder.replace(
      editor.selection,
      wrapSelectionInFragment(editor.document.getText(editor.selection)),
    );
  });
}
...
async function tabOut() {
  ...
  const nextSelections = editor.selections.map((selection) => {
    ...
    const nextCharacter = findTabOutPosition(
      lineText,
      selection.active.character,
      getTabOutCharacterSets(),
    );
...
async function tabOutReverse() {
  ...
  const nextSelections = editor.selections.map((selection) => {
    ...
    const lineText = editor.document.lineAt(selection.active.line).text;
    const nextCharacter = findReverseTabOutPosition(
      lineText,
      selection.active.character,
      getTabOutCharacterSets(),
    );
...
async function toggleTabOut() {
  const configuration = vscode.workspace.getConfiguration("gossamer-utils");
  ...
  vscode.window.showInformationMessage(
    `Gossamer Utils TabOut is ${enabled ? "disabled" : "enabled"}.`,
  );
}
...
async function hungryDelete() {
  ...
  if (
    !vscode.workspace
      .getConfiguration("gossamer-utils")
      .get<boolean>("hungryDelete.enabled", true)
  ) {
...
class ScriptCodeActionProvider implements vscode.CodeActionProvider {
  provideCodeActions(
    ...
    if (
      actionRange.isEmpty &&
      !document.getWordRangeAtPosition(actionRange.start)
    ) {
      return [];
    }

    const actions = [
      createCommandCodeAction("Rename Symbol", "gossamer-utils.renameSymbol"),
      createCommandCodeAction(
        "Go to Definition",
        "gossamer-utils.goToDefinition",
      ),
    ];
    const functionToArrowEdit = createFunctionToArrowEdit(
      document.getText(),
      document.offsetAt(actionRange.start),
    );
    const arrowToFunctionEdit = createArrowToFunctionEdit(
      document.getText(),
      document.offsetAt(actionRange.start),
    );
...
function getFrameworkMode(): FrameworkMode {
  return vscode.workspace
    .getConfiguration("gossamer-utils")
    .get<FrameworkMode>("framework", "react");
}
...
function createCommandCodeAction(
  title: string,
  command: string,
  args: unknown[] = [],
): vscode.CodeAction {
...
function isTabOutEnabled(): boolean {
  return vscode.workspace
    .getConfiguration("gossamer-utils")
    .get<boolean>("tabOut.enabled", true);
}

function getTabOutCharacterSets(): TabOutCharacterSet[] {
  return vscode.workspace
    .getConfiguration("gossamer-utils")
    .get<TabOutCharacterSet[]>("tabOut.characters", defaultTabOutCharacterSets);
}
