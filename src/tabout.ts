...
export function findTabOutPosition(
	...
	const previousCharacterSet = findCharacterSet(previousCharacter, characterSets);

	if (!previousCharacterSet) {
		return undefined;
	}

	const nextMatchingCharacter = findNextMatchingCharacter(lineText, position + 1, previousCharacterSet);

	if (nextMatchingCharacter !== undefined) {
		return nextMatchingCharacter;
	}

	const nextSpecialCharacter = findNextSpecialCharacter(lineText, position + 1, characterSets);
